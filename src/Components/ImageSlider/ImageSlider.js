	...
	useEffect(() => {
		...
		if (product.images) {
			document.addEventListener(
				...
				(e) => {
					if (e.keyCode === 39) {
						if (newCurrent.current === product.images.length - 1)
							newCurrent.current = 0;
						else newCurrent.current += 1;
						return setCurrent(newCurrent.current);
					}
					if (e.keyCode === 37) {
						if (newCurrent.current === 0)
							newCurrent.current = product.images.length - 1;
						else newCurrent.current -= 1;
						return setCurrent(newCurrent.current);
					}
				},
