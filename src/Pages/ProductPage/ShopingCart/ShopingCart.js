...
const ShopingCart = ({ product, loading, isDesktop, isMobile }) => {
	...
	useEffect(() => {
		...
		if (t1.current) {
			t1.current.to(productInfoRef.current, {
				opacity: 1,
				scale: 1,
				duration: 0.6,
			});
			...
			<div style={{ padding: '2.5vw' }}>
				...
				<StyledSizesAndInfo>
					<StyledSizeAndError isClicked={isClicked}>
						...
						{error ? (
							<StyledError>Please select your size !</StyledError>
						) : null}
					</StyledSizeAndError>
					...
					<StyledInfo>
						...
						<StyledInfoItem>
							...
							JOIN CREATORS CLUB TO GET UNLIMITED FREE STANDARD SHIPPING,
							RETURNS & EXCHANGES
						</StyledInfoItem>
					</StyledInfo>
				</StyledSizesAndInfo>
