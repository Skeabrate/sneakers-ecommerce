...
const Payment = () => {
	...
	const handleSubmit = (e) => {
		...
		if (rules) {
			...
			if (!state.expiration.value)
				reducerActionHandler('setIsActive', 'expiration');
			...
			{succesful ? (
				...
				<form onSubmit={(e) => handleSubmit(e)}>
					<CustomInput
						...
						setActiveError={() =>
							!state.name.isActive &&
							reducerActionHandler('setIsActive', 'name')
						}
					...
					<CustomInput
						...
						pattern="[\d| ]{16,22}"
						onChange={(e) =>
							reducerActionHandler('setValue', 'card', e.currentTarget.value)
						}
						setActiveError={() =>
							!state.card.isActive &&
							reducerActionHandler('setIsActive', 'card')
						}
					...
					<StyledCardInfo>
						...
						<CustomInput
							...
							pattern="\d{3,4}"
							onChange={(e) =>
								reducerActionHandler('setValue', 'code', e.currentTarget.value)
							}
							setActiveError={() =>
								!state.code.isActive &&
								reducerActionHandler('setIsActive', 'code')
							}
